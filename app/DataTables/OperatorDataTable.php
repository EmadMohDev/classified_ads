...
class OperatorDataTable extends DataTable
{
    ...
    public function html()
    {
        ...
        return $this->builder()
                    ...
                    ->pageLength(5)
                    ->buttons([
                        Button::make()->text('<i class="fa fa-plus"></i> <span class="hidden" data-yajra-href="'.$create_route.'"></span>')->addClass('btn btn-info show-modal-form '. (canUser("operators-create") ? "" : "remove-hidden-element"))->titleAttr(trans('menu.create-row', ['model' => trans('menu.operator')])),
                        Button::make()->text('<i class="fas fa-trash"></i>')->addClass('btn btn-danger multi-delete '. (canUser("operators-multidelete") ? "" : "remove-hidden-element"))->titleAttr(trans('buttons.multi-delete')),
