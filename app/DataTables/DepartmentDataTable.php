...
class DepartmentDataTable extends DataTable
{
    ...
    public function html()
    {
        ...
        ->pageLength(5)
        ->buttons([
            Button::make()->text('<i class="fa fa-plus"></i>')->addClass('btn btn-info '. (canUser("departments-create") ? "" : "remove-hidden-element"))->action("window.location.href = " . '"' . routeHelper('departments.create') . '"')->titleAttr(trans('menu.create-row', ['model' => trans('menu.department')])),
            Button::make()->text('<i class="fas fa-trash"></i>')->addClass('btn btn-danger multi-delete '. (canUser("departments-multidelete") ? "" : "remove-hidden-element"))->titleAttr(trans('buttons.multi-delete')),
