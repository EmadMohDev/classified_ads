...
class AggregatorDataTable extends DataTable
{
    ...
    public function html()
    {
        ...
        ->pageLength(5)
        ->buttons([
            Button::make()->text('<i class="fa fa-plus"></i> <span class="hidden" data-yajra-href="'.routeHelper('aggregators.create').'"></span>')->addClass('btn btn-info show-modal-form '. (canUser("aggregators-create") ? "" : "remove-hidden-element"))->titleAttr(trans('menu.create-row', ['model' => trans('menu.aggregator')])),
            Button::make()->text('<i class="fas fa-trash"></i>')->addClass('btn btn-danger multi-delete '. (canUser("aggregators-multidelete") ? "" : "remove-hidden-element"))->titleAttr(trans('buttons.multi-delete')),
