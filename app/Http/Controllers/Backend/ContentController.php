...
class ContentController extends BackendController
{
    ...
    public function update(ContentRequest $request, ContentService $contentService, $id)
    {
        ...
        if (is_string($content)) return $this->throwException($content);
        return $this->redirect(trans('flash.row updated', ['model' => trans('menu.content')]), routeHelper("contents.show", $content));
    }
    ...
    public function getTypeInput(Request $request)
    {
        ...
        return $view_path ? view($view_path, compact('row'), ['content_type_id' => $request->content_type_id]) : '';
    }
